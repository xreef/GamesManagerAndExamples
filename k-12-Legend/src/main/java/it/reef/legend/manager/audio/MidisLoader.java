package it.reef.legend.manager.audio;
...
import it.reef.legend.element.action.SoundsWatcher;
...
public class MidisLoader implements MetaEventListener {
	private static final Log log = LogFactory.getLog(MidisLoader.class);
	 ...
	 */
	{
		...
		try {
			...
			if (sequencer == null) {
				log.error("Cannot get a sequencer");
				return;
			}
			...
			if (!(sequencer instanceof Synthesizer)) {
				log.info("Linking the MIDI sequencer and synthesizer");
			...
			log.error("No sequencer available");
			sequencer = null;
		}
...
//					fnm = 
			...
			log.error("Error reading file: " + sndsFNm);
			System.exit(1);
		}
		return mtl;
	}
	...
	// create a MidiInfo object, and store it under name
	{
		if (midisMap.containsKey(name))
			log.warn("Error: " + name + "already stored");
		else if (sequencer == null)
			log.error("No sequencer for: " + name);
		else {
			if (classForStream!=null){
	...
	// play (perhaps loop) the sequence
	{
		...
		if (mi == null)
			...
			if (currentMidi != null)
				log.warn("Sorry, " + currentMidi.getName()
						+ " already playing");
			else {
	 ...
	 */
	{
		if (meta.getType() == END_OF_TRACK) {
			...
			log.info("  END_OF_TRACK for " + name);
