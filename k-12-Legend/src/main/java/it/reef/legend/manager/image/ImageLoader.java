package it.reef.legend.manager.image;


import it.reef.legend.manager.LoaderWatcher;
...
public class ImageLoader {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (ch == 'o') // a single image
					imageNum++;
				if (ch == 'i') // a single image positioning
					imageNum++;
					...
					imageNum++;
				else if (ch == 'p') // a positioning images strip
					imageNum++;
				else
	 ...
	 */
	{
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				else if (ch == 'i') // a single image positioning
				{
					getFileNameImagePositioning(line);
					if (this.ilw!=null) this.ilw.setElementiCaricati(immaginiCaricate++, "Immagine semplice");
				}
				else if (ch == 'n') // a numbered sequence of images
					{
					...
					{
						getStripImagesPositionAndSize(line);
						if (this.ilw!=null) this.ilw.setElementiCaricati(immaginiCaricate++, "Immagine posizionale composta");
					}
				else if (ch == 'm') // an images strip
					{
	...
	private void getFileNameImagePositioning(String line)
	/*
	 * format: o <fnm>
	 */
	{
		...
		if (tokens.countTokens() != 7)
		   ...
		   { log.error("Number Y is incorrect for " + line);  }
		   ...
		   { log.error("Number X is incorrect for " + line);  }
		   ...
		   { log.error("Number height is incorrect for " + line);  }
		   ...
		   { log.error("Number width is incorrect for " + line);  }		
	       
		   loadSingleImagePositioning(name, fnm, X, Y, width, height);
		}
	} // end of getFileNameImage()

	public boolean loadSingleImagePositioning(String name, String fnm, int X, int Y, int width, int height)
	// can be called directly
	{
		if (immagini.containsKey(name)) {
			log.error("Error: " + name + "already used");
	     ...
	     BufferedImage strip;
	     Graphics2D stripGC;
	     int allWidth = (int) stripIm.getWidth();
	     int allHeight = stripIm.getHeight();
	     int transparency = stripIm.getColorModel().getTransparency();

	     if (X+width > allWidth){
	    	 log.error("X+width > allWidth");
	   		 return false;
	   	 }
	   	 if (Y+height > allHeight){
	   		log.error("Y+height > allHeight");
	   		return false;
	   	 }
   	 
	   	 strip = gc.createCompatibleImage(width,height, transparency);
	   	 stripGC = strip.createGraphics();
		...
		log.info("  Stored " + name + "/" + fnm);
		return true;
	} // end of loadSingleImagePositioning()	
	...
	// --------- load numbered images -------------------------------
	   ...
	   */
	   ...
	   // --------- load image strip positioning and size-------------------------------

	   private void getStripImagesPositionAndSize(String line)
	   /* format:
	         s <fnm> <Y> <X> <Height> <Width> <Column> <Row> <Total Number>
	   */
	   {
	     StringTokenizer tokens = new StringTokenizer(line);

	     if (tokens.countTokens() != 10)
	       log.error("Wrong no. of arguments for " + line);
	     else {
	       tokens.nextToken();    // skip command label
	       System.out.print("s Line: ");
	       ...
	       { log.error("Number columns is incorrect for " + line);  }
	       ...
	       { log.error("Number rows is incorrect for " + line);  }

	       int totImages = -1;
	       try {
	    	   totImages = Integer.parseInt( tokens.nextToken() );
	       }
	       catch(Exception e)
	       { log.error("Number totImages is incorrect for " + line);  }

	       
	       loadStripPositionImages(name, fnm, Y, X, height, width, columns, rows, totImages);
	     }
	   }  // end of getStripImages()

	   public int loadStripPositionImages(String name, String fnm, int Y, int X, int height, int width, int columns, int rows, int totImages)
	   /* Can be called directly, to load a strip file, <fnm>,
	      holding <number> images.
	   */
	   {
//	     String name = getPrefix(fnm);
	     ...
	     BufferedImage[] strip;
		try {
			strip = loadStripPositionImageArray(fnm, Y, X, height, width, columns, rows, totImages);
		} catch (Exception e) {
	   ...
	   public BufferedImage[]  loadStripPositionImageArray(String fnm, int Y, int X, int height, int width, int columns, int rows, int totImages) throws Exception 
	   /* Extract the individual images from the strip image file, <fnm>.
	      We assume the images are stored in a single row, and that there
	      are <number> of them. The images are returned as an array of
	      BufferedImages
	   */
	   {
	     ...
	     BufferedImage strip[] = new BufferedImage[totImages];
	     Graphics2D stripGC;

	     int imageLoaded = 0;
    	 for (int row=0; row<rows; row++){
    		 for (int column=0; column<columns; column++){
		    	 int startImgX = X + (column*width);
		    	 int startImgY = Y + (row*height);
		    	 
		    	 if (startImgX+width > allWidth){
		    		 throw new Exception("X grater then full size");
		    	 }
		    	 if (startImgY+height > allHeight){
		    		 throw new Exception("Y grater then full size");
		    	 }
		    	 
		    	 strip[imageLoaded] = gc.createCompatibleImage(width,height, transparency);
		    	 stripGC = strip[imageLoaded].createGraphics();
		    	 stripGC.drawImage(stripIm, 
		    			 	0,0, width,height,
		    			 	startImgX,startImgY, startImgX+width,startImgY+height,
		                   null);
		    	 stripGC.dispose();
		    	 
		    	 imageLoaded++;
		    	 if (imageLoaded == totImages){
		    		 return strip;
		    	 }
		     }
	     }
	   ...
	   */
	   {
	     ...
	     if (groupNames == null) {
	       log.error("No group names for " + name);  
	       return -1;
	     }
