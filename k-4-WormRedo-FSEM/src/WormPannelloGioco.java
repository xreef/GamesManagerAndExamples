...
public class WormPannelloGioco extends JFrame implements Runnable
{
...
protected int NUM_BUFFERS=2;
  ...
  public WormPannelloGioco( long period)
  {
...
private void gameRender(Graphics gScr)
{
    ...
    gScr.drawString(fpsUpsText, 20, 25);

    // report time used and boxes used at bottom left

    gScr.drawString(levelText+"  "+boxNumberText+"  "+timeSpentText, 10, PHEIGHT-15);
...
//    fred.draw(dbg);
	...
	wallManager.draw(gScr);
	vermicello.draw(gScr);
	fruit.draw(gScr);
    ...
    if (gameOver){
	    ...
	    if (isLevelComplete) {
	    	gameCompleteMessage(gScr);
	    }else if (isPaused){
	    	gamePauseMessage(gScr);
	    }
    }
}  // end of gameRender( )
...
{  
  ...
  new WormPannelloGioco(period*1000000L);    // ms --> nanosecs
}  // end of main()
