...
public class Worm implements Drawable, Updatable, KeyProcessableWorm {
	...
	public void draw(Graphics g) {
		Random r = new Random();
		if (collision){
			headColor = new Color(r.nextInt(255),r.nextInt(255),r.nextInt(255));
		}
		if (positionOfWorm.size() > 0) {
			if (collision) bodyColor = new Color(r.nextInt(255),r.nextInt(255),r.nextInt(255));
	...
	public void update() {
	    ...
	    if (positionOfWorm.size() == 0) {   // empty array at start
	    	positionOfWorm.add(new Point( DOTSIZE*25/2, DOTSIZE*20/2 )); // center pt
	    }
